...
public class UtilValidate {
    ...
    /** 
     * True if sting s is not a negtive double number
     * by andy 2013 12 24
     */
...
//    }
    ...
    public static Timestamp dateTransfor(String dateStr){
        if(dateStr!=null){
        	SimpleDateFormat df1 = new SimpleDateFormat("yyyy-MM-dd");
            SimpleDateFormat df2 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
            Date tmp = new Date();
            try{
            	tmp = new Date(df1.parse(dateStr).getTime());
            	
            	return Timestamp.valueOf(df2.format(tmp));
            }catch(Exception e){
                e.printStackTrace();
            }
        }
